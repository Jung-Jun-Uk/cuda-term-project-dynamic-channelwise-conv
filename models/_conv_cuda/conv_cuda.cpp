...
std::vector<torch::Tensor> conv_forward(torch::Tensor input,
    ...
    for(int elt = 0; elt < batch_size; elt++) {
      ...
      if(is_bias) {
        output[elt] += bias.mm(ones).reshape(torch::IntArrayRef({nOutputPlane, outputHeight, outputWidth})).cuda();
      }
      ...
      output[elt] += weights.mm(columns).reshape(torch::IntArrayRef({nOutputPlane, outputHeight, outputWidth})).cuda();
    }
    return {output};                                       
}
...
torch::Tensor backward_gradInput(torch::Tensor input,
    ...
    weights = weights.reshape(torch::IntArrayRef({nOutputPlane, nInputPlane*kW*kW})).t().cuda();
    ...
    for(int elt = 0; elt < batch_size; elt++) {
      ...
      // gradcolumns.dim: (inplanes * kW * kH) * (outHeight * outWidth)
      // col2im: gradcolumns.dim -> (inplanes) * (inHeight) * (inWidth)
      gradInput[elt] += torch::col2im(gradColumns.clone(), /*output_size=*/torch::IntArrayRef({inputHeight, inputWidth}),
                                                           /*kernel_size=*/torch::IntArrayRef({kW, kH}),
                                                           /*dilaton=*/torch::IntArrayRef({1,1}),
                                                           /*padding*/torch::IntArrayRef({padW,padH}),
                                                           /*stride*/torch::IntArrayRef({dW,dH})).cuda();
    }
...
std::vector<torch::Tensor> backward_gradParameters(torch::Tensor input,
    ...
    for(int elt = 0; elt < batch_size; elt++) {
      ...
      gradWeights += gradOutput_n.mm(columns).reshape(torch::IntArrayRef({nOutputPlane, nInputPlane, kW, kH}));

      if(is_bias) {
        gradBias += gradOutput_n.mm(ones).reshape(torch::IntArrayRef({nOutputPlane}));
      }
    }
